...
abstract class CarDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private fun fillInDb(context: Context) {
            ...
            Timber.d("testdb  fillInDb ")
            Executors.newSingleThreadExecutor().execute {
                getInstance(context).carDao().insertCars(
                    CHEESE_DATA.map {
                        carBuilder {
                            carName = it;
                            year = (1960..2019).shuffled().first().toString()
                        }
                    }).subscribe({}, { error -> Timber.d("testdb ${error}") })
            }
        }

        //(1960..2019).shuffled().first().toString()
    }
